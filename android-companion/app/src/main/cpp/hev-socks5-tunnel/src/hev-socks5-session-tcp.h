...
#ifdef ANDROID
    /* Per-connection app identity, resolved via JNI at construction time
     * (see hev-jni.c / hev-socks5-session-tcp.c). NULL when unresolved
     * (pre-Android-10 device, lookup failure, or race during teardown) --
     * hev_socks5_session_run() falls back to the tunnel's static
     * socks5.username/password config in that case, same as before this
     * field existed. */
    char *app_user;
    char *app_pass;
#endif
