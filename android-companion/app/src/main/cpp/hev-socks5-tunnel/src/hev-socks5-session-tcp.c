...
#ifdef ANDROID
static void
hev_socks5_session_tcp_resolve_identity (HevSocks5SessionTCP *self,
                                         struct tcp_pcb *pcb)
{
    char local_addr[IPADDR_STRLEN_MAX];
    char remote_addr[IPADDR_STRLEN_MAX];
    char *device_id;
    char *package_id;
    int res;

    /* lwip's tcp_pcb "local" is actually the connection's real destination
     * (lwip impersonates that destination from the tun interface's point
     * of view) and its "remote" is the device app's own real source
     * socket. getConnectionOwnerUid() wants (local=app's socket,
     * remote=destination), so the two must be swapped here. */
                                  ...
                                  struct tcp_pcb *pcb, HevTaskMutex *mutex)
{
    ...
    self->app_user = NULL;
    self->app_pass = NULL;
    hev_socks5_session_tcp_resolve_identity (self, pcb);
#endif
...
#ifdef ANDROID
    if (self->app_user)
        hev_free (self->app_user);
    if (self->app_pass)
        hev_free (self->app_pass);
#endif
